	...
	render = () => {
		...
		return (
			<Link key={this.state.props.content.id} as={`/directors/${this.state.props.content.slug}/${this.state.props.content.id}`} href={`/directors?id=${this.state.props.content.id}`}>
				...
				<a className="link-directors__item-container" onMouseOver={() => this.hoverLinks(this.state.props.content.videoMp4 !== 'default' ? this.state.props.content.videoMp4.url : '-')}>
